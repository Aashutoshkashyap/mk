      ...
      <section className="relative pt-28 pb-16 md:pt-36 md:pb-24 bg-primary overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 relative z-10">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="text-center">
            <span className="text-xs font-bold tracking-widest uppercase text-brand-blue">{about?.subheading || "Who We Are"}</span>
            <h1 className="mt-4 font-display text-4xl md:text-5xl font-extrabold text-primary-foreground leading-tight">About Sharp Edge</h1>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="grid md:grid-cols-2 gap-8">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={visionInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="rounded-2xl bg-card border border-border p-8 md:p-10">
              <div className="w-14 h-14 rounded-xl bg-primary/[0.08] flex items-center justify-center mb-6"><Eye size={28} className="text-primary" strokeWidth={1.5} /></div>
              <h3 className="font-display text-2xl font-bold text-primary mb-4">{about?.vision_title || "Our Vision"}</h3>
              <p className="text-muted-foreground leading-relaxed">{about?.vision_text || ""}</p>
            </motion.div>
            <motion.div initial={{ opacity: 0, y: 30 }} animate={visionInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.15 }} className="rounded-2xl bg-card border border-border p-8 md:p-10">
              <div className="w-14 h-14 rounded-xl bg-primary/[0.08] flex items-center justify-center mb-6"><Target size={28} className="text-primary" strokeWidth={1.5} /></div>
              <h3 className="font-display text-2xl font-bold text-primary mb-4">{about?.mission_title || "Our Mission"}</h3>
              <p className="text-muted-foreground leading-relaxed">{about?.mission_text || ""}</p>
            </motion.div>
          </div>
        </div>
      </section>
      ...
      <section ref={galleryRef} className="py-20 md:py-28 bg-secondary/50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={galleryInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }} className="text-center mb-14">
            ...
            <h2 className="mt-3 font-display text-3xl md:text-4xl font-extrabold text-primary">Gallery</h2>
          </motion.div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {gallery.map((img: any, i: number) => (
              <motion.div key={img.id} initial={{ opacity: 0, scale: 0.9 }} animate={galleryInView ? { opacity: 1, scale: 1 } : {}} transition={{ duration: 0.4, delay: 0.06 * i }} className="rounded-xl overflow-hidden aspect-[4/3] group">
