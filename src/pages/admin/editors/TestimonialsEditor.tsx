      ...
      <div className="space-y-4">
        {items.map((t) => (
          <div key={t.id} className="rounded-2xl bg-card border border-border p-5 space-y-3">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Photo URL (optional)</label>
                <input value={t.image_url || ""} onChange={(e) => update(t.id, "image_url", e.target.value)} placeholder="https://..."
                  className="w-full rounded-lg border border-border bg-secondary/50 px-3 py-2 text-sm" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1">Rating (1–5)</label>
                <input type="number" min={1} max={5} value={t.rating} onChange={(e) => update(t.id, "rating", parseInt(e.target.value))}
                  className="w-full rounded-lg border border-border bg-secondary/50 px-3 py-2 text-sm" />
              </div>
            </div>
